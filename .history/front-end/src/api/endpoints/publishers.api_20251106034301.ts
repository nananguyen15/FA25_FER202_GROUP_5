...
export const publishersApi = {
  ...
  getAll: async (): Promise<Publisher[]> => {
    const response = await apiClient.get<ApiResponse<Publisher[]>>(PUBLISHERS_ENDPOINT);
    return response.data.result;
  },

  // GET active publishers
  getActive: async (): Promise<Publisher[]> => {
    const response = await apiClient.get<ApiResponse<Publisher[]>>(`${PUBLISHERS_ENDPOINT}/active`);
    return response.data.result;
  },
  ...
  getById: async (id: number): Promise<Publisher> => {
    const response = await apiClient.get<ApiResponse<Publisher>>(`${PUBLISHERS_ENDPOINT}/${id}`);
    return response.data.result;
  },
};
