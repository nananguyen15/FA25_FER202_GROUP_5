...
export function PublisherManagement() {
  ...
  const [publisherBooks, setPublisherBooks] = useState<any[]>([]);
  ...
  const handleViewBooks = async (publisher: Publisher) => {
    try {
      ...
      const books = await publishersApi.getBooksByPublisherId(publisher.id);
      setPublisherBooks(books);
      setShowBooksModal(true);
    } catch (error) {
      console.error("Error loading books:", error);
      alert("Không thể tải danh sách sách của nhà xuất bản");
    }
  };
