...
export function StaffManagement() {
  ...
  const handleCreate = async () => {
    try {
      ...
      // Prepare create data - no password needed
      const createData = {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // Prepare update data
      // Note: Email should not be changed (readOnly in form)
      // Full name is optional in update
      const updateData = {
        ...formData,
        username: formData.username.trim(),
        name: formData.name ? formData.name.trim() : "", // Optional
        phone: formData.phone ? formData.phone.trim() : "",
        address: formData.address ? formData.address.trim() : "",
      };
