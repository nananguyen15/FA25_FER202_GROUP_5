...
import { SupCategory, CreateSupCategoryRequest, UpdateSupCategoryRequest } from "../../types/api/category.types";
...
export function SupCategoryManagement() {
  ...
  // Form data
  const [supFormData, setSupFormData] = useState<CreateSupCategoryRequest>({
    name: "",
    slug: "",
  });
  ...
  const loadSupCategories = async () => {
    ...
    try {
      ...
      setSupCategories(response.data || []);
    } catch (error) {
  ...
  const handleUpdateSup = async (e: React.FormEvent) => {
    ...
    try {
      await categoriesApi.sup.update(selectedSup.id, supFormData as UpdateSupCategoryRequest);
