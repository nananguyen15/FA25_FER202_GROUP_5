...
export function PublisherManagement() {
  ...
  const handleCreate = async () => {
    try {
      ...
      // Prepare clean data (without image)
              ...
              paginatedPublishers.map((publisher) => (
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <img
                      src={transformImageUrl(publisher.image) || FALLBACK_IMAGES.publisher}
                      alt={publisher.name}
                      className="object-cover w-16 h-16 border-2 border-gray-200 rounded"
                      onError={(e) => {
                        e.currentTarget.src = FALLBACK_IMAGES.publisher;
                      }}
                    />
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
          ...
          <ViewDetailsContainer>
            {selectedPublisher.image && (
              <div className="flex justify-center mb-4">
                <img
                  src={transformImageUrl(selectedPublisher.image) || FALLBACK_IMAGES.publisher}
