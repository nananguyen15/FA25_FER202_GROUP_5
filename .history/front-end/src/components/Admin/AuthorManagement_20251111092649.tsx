...
export function AuthorManagement() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // Filter out empty strings - send only non-empty fields
      const updateData: any = {
        name: formData.name,
      };
      
      if (formData.biography && formData.biography.trim()) {
        updateData.biography = formData.biography.trim();
      }
      ...
      alert(`Không thể cập nhật tác giả: ${error.response?.data?.message || error.message}`);
    }
  };
