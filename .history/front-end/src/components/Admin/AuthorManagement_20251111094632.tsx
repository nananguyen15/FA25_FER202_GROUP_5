...
export function AuthorManagement() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // IMPORTANT: Send empty string for biography if not provided
      // Backend might require all 3 fields to be present
      const updateData = {
        name: trimmedName,
        biography: formData.biography?.trim() || "",  // Send empty string instead of null
        image: formData.image?.trim() || "",  // Send empty string instead of null
      };
      ...
      alert(`Không thể cập nhật tác giả: ${errorMsg}\n\nĐây là lỗi từ Backend. Vui lòng kiểm tra Backend logs!`);
    }
  };
