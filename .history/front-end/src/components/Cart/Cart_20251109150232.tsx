...
export function Cart() {
  ...
  const handleQuantityBlur = async (item: any) => {
    ...
    const bookDetail = bookDetails[item.bookId];

    // Validate: must be positive
    if (newQuantity <= 0) {
      setQuantityError((prev) => ({
        ...prev,
        [item.id]: "Quantity must be at least 1",
      }));
      setEditingQuantity((prev) => {
        const newState = { ...prev };
        delete newState[item.id];
    ...
    // Validate: not exceed stock if we have book details
    if (bookDetail && newQuantity > bookDetail.stockQuantity) {
      setQuantityError((prev) => ({
        ...prev,
        [item.id]: `Only ${bookDetail.stockQuantity} available in stock`,
      }));
      setEditingQuantity((prev) => {
        const newState = { ...prev };
        delete newState[item.id];
    ...
    if (newQuantity !== item.quantity) {
      try {
        ...
        if (err?.response?.data?.message?.includes("exceeds") || 
            err?.response?.data?.message?.includes("stock")) {
          setQuantityError((prev) => ({
            ...prev,
            [item.id]: err.response.data.message,
          }));
        } else {
          setQuantityError((prev) => ({
            ...prev,
            [item.id]: "Failed to update quantity. Please try again.",
          }));
        }
      }
    }
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                  ...
                  >
                    ...
                    <div className="flex flex-col justify-between flex-grow">
                      ...
                      <div className="mt-4">
                        <div className="flex items-center border rounded-md w-fit">
                          <button
                            onClick={() => {
                              ...
                              // Clear error when using buttons
                              setQuantityError((prev) => {
                                const newState = { ...prev };
                                delete newState[item.id];
                                return newState;
                          ...
                          <input
                            ...
                            className={`w-16 px-2 py-1 text-center border-x focus:outline-none focus:ring-2 ${
                              quantityError[item.id]
                                ? "focus:ring-red-500 border-red-300"
                                : "focus:ring-beige-500"
                            }`}
                          />
                          <button
                            onClick={() => {
                              ...
                              if (bookDetail && newQty > bookDetail.stockQuantity) {
                                setQuantityError((prev) => ({
                                  ...prev,
                                  [item.id]: `Only ${bookDetail.stockQuantity} available in stock`,
                                }));
                        ...
                        {/* Error message below quantity input */}
                        {quantityError[item.id] && (
                        ...
                        {/* Stock info hint */}
                        {bookDetails[item.bookId] && !quantityError[item.id] && (
                          <p className="mt-1 text-xs text-beige-500">
                            {bookDetails[item.bookId].stockQuantity} in stock
                          </p>
                        )}
                      </div>
                    </div>

                    {/* Price and Remove on the right */}
              ...
              <div className="p-6 bg-white rounded-lg shadow-sm lg:col-span-1 h-fit">
                ...
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-xl font-bold text-beige-900">
                    Price Details
                  </h2>
                ...
                {selectedItemsCount === 0 ? (
                  ...
                  <>
                    {/* List selected items in table format with aligned columns */}
                    <div className="mb-4 space-y-3">
                      {cart?.cartItems
                        ...
                        .map((item) => (
                          ...
                          >
                            {/* Quantity - fixed width, right aligned */}
                            <span className="text-right text-beige-700 min-w-[2rem]">
                              {item.quantity}
                            </span>
                            {/* x symbol - fixed width, centered */}
                            <span className="text-center text-beige-600 w-4">
                              x
                            </span>
                            {/* Book title - flexible width */}
                            <span className="text-beige-700">
                              {item.bookTitle}
                            </span>
                            {/* Price - fixed width, right aligned */}
                            <span className="font-semibold text-right text-beige-900 min-w-[4rem]">
                              ${item.subtotal.toFixed(2)}
                            </span>
                          </div>
                        ))}
                    ...
                    <Link to="/checkout">
                      <button className="flex items-center justify-center w-full gap-2 py-3 font-semibold text-white transition-colors rounded-lg bg-beige-900 hover:bg-beige-800">
                        Place order
                        <svg
                          className="w-5 h-5"
                          fill="none"
