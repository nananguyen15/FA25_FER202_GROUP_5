...
export function StaffProfile() {
  ...
  const mergeAddress = (street: string, ward: string, district: string, province: string) => {
    ...
    const loadProfile = async () => {
      try {
        ...
        if (addressParts.province) {
          const provinceData = provinces.find((p) => p.name === addressParts.province);
          if (provinceData) {
            const filteredDistricts = districts.filter(
              (d) => d.province_code === provinceData.code
            );
            ...
            if (addressParts.district) {
              const districtData = districts.find((d) => d.name === addressParts.district);
              if (districtData) {
                const filteredWards = wards.filter(
                  (w) => w.district_code === districtData.code
                );
    ...
    <div className="p-6 bg-white rounded-lg shadow-sm">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="pt-6 mt-6 border-t border-beige-200">
          <h3 className="mb-4 text-lg font-semibold text-beige-800">Address Information</h3>
          ...
          <div>
            ...
            {errors.province && (
              <p className="mt-2 text-sm text-red-600">{errors.province.message}</p>
            )}
          </div>
