...
export function StaffProfile() {
  ...
  const onSubmit = async (data: ProfileFormValues) => {
    try {
      ...
      console.log("📝 Submitting staff profile update:", {
        name: data.name,
        phone: data.phone,
      ...
      if (error.response?.data?.message) {
        ...
        errorMessage =
          "Invalid data. Please check your phone number format (10-11 digits starting with 0).";
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            value={user?.username || ""}
            disabled
            className="block w-full px-3 py-2 mt-1 border rounded-md shadow-sm cursor-not-allowed bg-beige-50 border-beige-300 text-beige-600"
          />
        </div>
