...
export default function SearchSuggest() {
  ...
  useEffect(() => {
    ...
    const normalizedQ = normalizeQuery(trimmedQ);

    // Use normalized query for cache key
    const cached = cache.current.get(normalizedQ);
    if (cached) {
    ...
    Promise.all([
      booksApi.search(normalizedQ).catch(() => [] as Book[]),
      authorsApi.search(normalizedQ).catch(() => [] as Author[]),
    ])
      .then(([books, authors]) => {
  ...
  function navigateToSearchResults(query: string) {
    setOpen(false);
    setHighlight(-1);
    // Normalize and navigate to books page with search query parameter
    const normalizedQuery = normalizeQuery(query);
    navigate(`/books?search=${encodeURIComponent(normalizedQuery)}`);
  }
            ...
            {items.map((it, idx) => (
              <li
                ...
                className={`px-4 py-3 cursor-pointer transition-colors border-b border-beige-100 last:border-b-0 ${
                  highlight === idx ? "bg-beige-100" : "hover:bg-beige-50"
                }`}
              >
          ...
          <div className="px-4 py-3 bg-beige-50 border-t border-beige-200">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="text-beige-600">
                Press{" "}
                <kbd className="px-1.5 py-0.5 text-xs font-semibold bg-white border border-beige-300 rounded">
                  Enter
                </kbd>{" "}
                to see all
              </span>
