...
export function ImageUpload({
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Generate path that matches DB format
    // DB format: /src/assets/img/{folder}/{filename}
    const dbPath = `/src/assets/img/${folder}/${filename}`;

    // Save the DB format to form state
    onChange(dbPath);

    // NOTE: In production, you need to:
    // 1. Upload file to server
    // 2. Server saves file to src/assets/img/{folder}/ directory  
    // 3. Server returns the DB path: /src/assets/img/{folder}/{filename}
    // 4. Frontend saves this path to the form state
    
    console.log(`📁 Image will be saved with path: ${dbPath}`);
    console.log(`🔗 This path will be stored in database`);
    console.log(`⚠️  Make sure to save the actual file to: src/assets/img/${folder}/${filename}`);
  };
  ...
  const handleUrlSubmit = () => {
    ...
    try {
      ...
      // If starts with /img/, convert to /src/assets/img/
  ...
  return (
    <div className="space-y-3">
      ...
      {(previewUrl || value) && (
        <div className="relative inline-block">
          <img
            ...
            onError={(e) => {
              e.currentTarget.src = "/src/assets/img/avatar/sample-user-avatar.png";
            }}
          />
          <button
            type="button"
      ...
      <p className="text-xs text-gray-500">
        {uploadMode === "file"
          ? "Upload from your computer. File will be saved to src/assets/img/ folder."
          : "Enter a full URL (https://...) or path (/src/assets/img/book/image.jpg)"}
      </p>
    </div>
  );
}
