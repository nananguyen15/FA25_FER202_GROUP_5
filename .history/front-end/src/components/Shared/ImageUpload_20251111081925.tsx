 ...
 * Saves to /img/{type}/ folder in public directory
 */
export function ImageUpload({
  ...
  const getImageFolder = () => {
    // This will be determined by parent component context
    // For now, return a default
    return "book"; // Can be: book, author, publisher, avatar, series
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Generate public URL path (this is what gets saved to DB)
    const folder = getImageFolder();
    const publicPath = `/img/${folder}/${filename}`;

    // In real implementation, you would upload the file to server here
    // For now, we'll use the public path format
    onChange(publicPath);

    // NOTE: In production, you need to:
    // 1. Upload file to server/cloud storage
    // 2. Server saves file to public/img/{folder}/ directory
    // 3. Server returns the public URL: /img/{folder}/{filename}
    // 4. Frontend saves this URL to the form state
  };
