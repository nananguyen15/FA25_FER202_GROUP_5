...
export default function HeroSlider({
  ...
  // Auto slide - use callback ref pattern to avoid infinite loop
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      setCurrentSlide((prevSlide) => {
        ...
        setTimeout(() => {
          ...
          const targetScroll =
            booksData.length > 1
              ? (totalScrollable / (booksData.length - 1)) * nextIndex
              : 0;
    ...
    // scrollToSlide is intentionally not in deps - we inline the logic to avoid re-creating interval
