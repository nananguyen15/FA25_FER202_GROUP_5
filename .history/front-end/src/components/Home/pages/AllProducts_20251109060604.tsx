...
type SortOption = "all" | "newest" | "price-low" | "price-high" | "popular" | "oldest" | "title";
...
export function AllProducts({ defaultType }: AllProductsProps) {
  ...
  const allProducts = useMemo(
    () => [
      ...books.map((book) => ({ ...book, type: "book" as const, isActive: true })),
      ...inactiveBooks.map((book) => ({ ...book, type: "book" as const, isActive: false })),
    ],
    [books, inactiveBooks]
  );
      ...
      <main className="flex-1 px-16 py-8 bg-beige-50">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="text-sm text-beige-500">
            Showing {paginatedProducts.length} of {filteredProducts.length} {defaultType}s
