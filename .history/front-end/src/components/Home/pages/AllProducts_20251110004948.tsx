...
// Normalize search query: remove dots, extra spaces
function normalizeQuery(query: string): string {
  return query
    .toLowerCase()
    .replace(/\./g, "") // Remove dots (j.k. -> jk)
...
export function AllProducts({ defaultType }: AllProductsProps) {
  ...
  const searchQuery = rawSearchQuery ? normalizeQuery(rawSearchQuery) : "";
  ...
  // Fetch books from API with sorting or searching
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        ...
        if (searchQuery.trim()) {
          booksData = await booksApi.search(searchQuery);

          // For search, only get active data - no inactive books
          const [authorsData, publishersData] = await Promise.all([
            authorsApi.getActive(),
            publishersApi.getActive(),
          ]);

          // Map books with author/publisher names
          ...
          setInactiveBooks([]); // Clear inactive books for search results
        } else {
    ...
    fetchBooks();
  }, [sortBy, searchQuery]); // Re-fetch when sortBy or searchQuery changes
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 px-16 py-8 bg-beige-50">
        ...
        {searchQuery && (
          <div className="mb-6">
            <h1 className="mb-2 text-2xl font-bold text-beige-900">
              Search Results
            </h1>
                  ...
                  {searchQuery ? (
                    <>
                      ...
                      <p className="mb-6 text-beige-600">
                        Không tìm thấy sách nào phù hợp với "{rawSearchQuery}"
                      </p>
                      <p className="mb-4 text-sm text-beige-500">Hãy thử:</p>
