          ...
          <div className="grid grid-cols-3 gap-6 p-8">
            {group.map((book) => (
              <div key={book.id} className="book-card">
                ...
                />
                <h3 className="mt-4 font-semibold text-brown-800">
                  {book.title}
                </h3>
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between py-4">
          ...
          <div className="flex gap-6">
            {categories.map((supCat) => (
              <div key={supCat.id} className="relative group">
                ...
                {subCategories[supCat.id] &&
                  subCategories[supCat.id].length > 0 && (
          ...
          <div className="flex gap-4">
            <Link to="/cart" className="hover:text-beige-200">
              Giỏ hàng
            </Link>
            <Link to="/auth/signin" className="hover:text-beige-200">
              Đăng nhập
            </Link>
          </div>
        </div>
    ...
    const fetchBookDetail = async () => {
      ...
      try {
        ...
        if (bookData.publisherId) {
          try {
            const publisherData = await publishersApi.getById(
        ...
        if (bookData.categoryId) {
          try {
            const categoryData = await categoriesApi.sub.getById(
              bookData.categoryId
            );
    ...
    <div className="container mx-auto px-4 py-12">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          {/* Title */}
          <h1 className="text-3xl font-bold text-brown-800">{book.title}</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`font-semibold ${
                book.stockQuantity > 0 ? "text-green-600" : "text-red-600"
              }`}
            >
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center border border-brown-300 rounded">
              ...
              <span className="px-6 py-2 border-x border-brown-300">
                {quantity}
              </span>
              <button
                onClick={() =>
                  setQuantity(Math.min(book.stockQuantity, quantity + 1))
                }
                className="px-4 py-2 hover:bg-brown-100"
