...
export default function SearchSuggest() {
  ...
  const allData = [
    ...heroBookGroups.flat().map(book => ({ id: book.id.toString(), label: book.title, type: "book" as const })),
    ...seriesData.map(series => ({ id: series.id.toString(), label: series.title, type: "series" as const })),
  ];
  ...
  return (
    <div className="relative w-full max-w-lg">
      ...
      />
      {loading && <div className="absolute text-sm text-gray-500 right-2 top-2"><IoMdSearch className="mr-3 w-7 h-7 text-beige-700" />Loading...</div>}

      {open && items.length > 0 && (
        <ul id="suggest-list" role="listbox" className="absolute z-10 w-full mt-1 bg-white border rounded shadow">
          {items.map((it, idx) => (
            <li
              key={it.id}
              id={`sugg-${idx}`}
      ...
      {open && !loading && items.length === 0 && (
        <div className="absolute z-10 w-full px-3 py-2 mt-1 text-gray-500 bg-white border rounded">No results</div>
      )}
    </div>
  );
}
