...
export default function SearchSuggest() {
  ...
  return (
    <div className="relative w-full max-w-lg">
      ...
      />
      {loading && <div className="absolute right-2 top-2 text-sm text-gray-500">Loading...</div>}

      {open && items.length > 0 && (
        <ul id="suggest-list" role="listbox" className="absolute z-10 mt-1 w-full bg-white border rounded shadow">
          {items.map((it, idx) => (
            <li
              key={it.id}
              id={`sugg-${idx}`}
      ...
      {open && !loading && items.length === 0 && (
        <div className="absolute z-10 mt-1 w-full bg-white border rounded px-3 py-2 text-gray-500">No results</div>
      )}
    </div>
  );
}
