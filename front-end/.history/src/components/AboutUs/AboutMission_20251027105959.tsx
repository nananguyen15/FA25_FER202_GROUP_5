      ...
      <div className="max-w-5xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-3">
          {missions.map((mission) => (
            <div key={mission.title} className="relative p-1 bg-gradient-to-r from-beige-300 to-beige-700 rounded-lg hover:from-beige-500 hover:to-beige-800 hover:animate-pulse transition-all duration-300">
              <div className="flex flex-col items-center p-5 bg-[var(--color-lightest)] rounded-lg">
                <div className="p-4 mb-4 rounded-full bg-beige-500 text-beige-100">
                  {mission.icon}
                </div>
                <h3 className="mb-2 text-2xl font-semibold text-[var(--color-darkest)]">{mission.title}</h3>
                <p className="leading-relaxed text-[var(--color-mid-dark)]">
                  {mission.description}
                </p>
              </div>
            </div>
