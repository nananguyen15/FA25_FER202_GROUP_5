      ...
      <div className="max-w-5xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 gap-10 md:grid-cols-3">
          {missions.map((mission) => (
            <div
              key={mission.title}
              className="flex flex-col items-center p-6 transition-colors duration-300 border rounded-lg border-beige-300 hover:border-beige-700"
            >
              <div className="p-4 mb-4 rounded-full bg-beige-500 text-beige-100">
                {mission.icon}
              </div>
              <h3 className="mb-2 text-2xl font-heading">{mission.title}</h3>
