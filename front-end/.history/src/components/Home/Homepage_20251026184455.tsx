...
export function Homepage() {
  ...
  return (
    ...
    >
      ...
      {Array.from({ length: Math.max(0, sectionCount - 1) }).map((_, idx) => (
        <section
          key={idx}
          className="w-full flex items-center justify-center px-6 py-8"
          aria-label={`Homepage section ${idx + 2}`}
        >
          {/* Thay bằng component / nội dung thực tế */}
          <div className="max-w-5xl w-full">
            {/* ...existing code... */}
            <div className="text-center text-beige-900">
              <h2 className="text-2xl font-bold mb-2">Section {idx + 2}</h2>
              <p className="text-beige-700">
                Nội dung demo cho hàng thứ {idx + 2}. Thay thế khi cần.
              </p>
            </div>
          </div>
        </section>
