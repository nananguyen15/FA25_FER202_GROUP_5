...
export default function HeroSlider({
  ...
  // Pre-calc itemHeight safely
  const itemHeight = useMemo(() => {
    const el = scrollContainerRef.current;
    if (!el) return 1; // avoid div-by-zero before mount
    return el.scrollHeight / Math.max(booksData.length, 1);
  }, [booksData.length]);
  ...
  return (
    <div className="w-full bg-beige-100">
      <div className="mx-auto max-w-screen-2xl px-6 lg:px-24 py-6 lg:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-12 items-center gap-8 lg:min-h-[50vh]">
          {/* Left */}
          <div className="lg:col-span-5">
            <h1 className="font-brand text-[clamp(2.5rem,5vw,4rem)] text-beige-900 font-bold mb-6">
              Find Your Next Book
            </h1>
            <p className="font-body text-[1.1rem] leading-relaxed text-beige-700 mb-8">
              Our most popular and trending <strong>BookVerse</strong> picks. Not sure what to read next? Start from your mood.
            </p>
            {/* React Router Link (no reload) */}
            <Link
              to={onGetStartedHref}
              className="inline-flex items-center justify-center bg-beige-700 text-beige-100 px-6 py-3 font-medium tracking-wide transition-all duration-300 hover:bg-beige-800 active:translate-y-[1px]"
            >
              Get Started
            </Link>
          </div>
          ...
          <div className="lg:col-span-7">
            <div className="flex items-center gap-4">
              ...
              <div
                ...
                className="hide-scrollbar h-[390px] flex flex-col gap-8 overflow-y-scroll scroll-smooth flex-1"
              >
                        ...
                        >
                          ...
                          <div className="relative w-[180px] h-[300px] mx-auto mb-4">
                            <div
                              ...
                              style={{
                                borderRadius: isCenter ? '0 0 100px 100px' : '100px 100px 0 0',
                              }}
                            >
                              <img
                                src={book.image ?? ''}
                          ...
                          {/* Info */}
                          <div className={`max-w-[200px] ${isCenter ? 'mx-auto mb-4' : 'mx-auto'}`}>
                            <h5 className="text-[1.1rem] font-bold text-beige-900 mb-1">{book.title}</h5>
                            <p className="text-[0.95rem] text-beige-700 m-0">{book.description.slice(0, 60)}...</p>
                          </div>
                        </div>
                      );
                    })}
                  ...
                  return (
                    <button
                      ...
                      className={`h-3 w-3 rounded-full border transition-all ${
                        active ? 'bg-beige-700 border-beige-700' : 'bg-transparent border-beige-700/30'
                      }`}
                    />
                  );
                })}
