...
export function Allbooks() {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (filters.discount.length > 0) {
      result = result.filter(product => {
        // Mock discount calculation (you can enhance this)
        return true;
      });
    }
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          <div className="relative">
            <select
              value={sortBy === 'price-low' ? 'price-low' : sortBy === 'price-high' ? 'price-high' : ''}
              onChange={(e) => setSortBy(e.target.value as SortOption)}
              className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg appearance-none cursor-pointer hover:bg-gray-50 pr-8"
            >
              <option value="">Price</option>
              <option value="price-low">Price: Low to High</option>
              <option value="price-high">Price: High to Low</option>
            </select>
            <span className="absolute text-gray-500 transform -translate-y-1/2 pointer-events-none right-3 top-1/2">▼</span>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredAndSortedProducts.map((product) => (
              <ProductCard
                ...
                id={product.id}
                title={product.title}
                author={(product as any).authorId || `Author ${(product as any).author_id}`}
                price={product.price}
