...
export function Navbar() {
  ...
  return (
    <nav className="flex flex-row items-center justify-between w-full px-10 py-3 bg-beige-100">
      {/* Brand text */}
      <h1 className="text-2xl font-bold mr-7 text-beige-900 font-brand">
        <a href="#">BookVerse</a>
      </h1>
          ...
          {links.map(([title, url]) => (
            <li key={title}>
              ...
              >
                ...
                <span className="block max-w-0 group-hover:max-w-full transition-all duration-200 h-0.5 bg-beige-500 mt-1" />
              </a>
            </li>
          ))}
        </ul>
