...
export function BookManagement() {
  ...
  const loadData = () => {
    ...
    // Load promotions
    const savedPromotions = JSON.parse(localStorage.getItem("adminPromotions") || "[]");
  ...
  const handleAddBook = () => {
    const newBook: Book = {
      ...
      status: formData.stockQuantity && formData.stockQuantity > 0 ? "In Stock" : "Out of Stock",
    };
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-beige-200">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as "id" | "price" | "date")}
              className="w-full px-4 py-2 border border-beige-300 rounded-lg focus:ring-2 focus:ring-beige-500 focus:border-beige-500"
            >
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-beige-100 border-b border-beige-200">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-semibold text-beige-700 uppercase">STT</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-beige-700 uppercase">ID</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-beige-700 uppercase">Cover</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-beige-700 uppercase">Title</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-beige-700 uppercase">Author</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-beige-700 uppercase">Category</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-beige-700 uppercase">Price</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-beige-700 uppercase">Stock</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-beige-700 uppercase">Status</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-beige-700 uppercase">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-beige-100">
              {currentBooks.length > 0 ? (
                currentBooks.map((book, index) => (
                  <tr key={book.id} className="hover:bg-beige-50 transition-colors">
                    <td className="px-4 py-4 text-sm text-beige-700">{startIndex + index + 1}</td>
                    <td className="px-4 py-4 text-sm font-medium text-beige-900">{book.id}</td>
                    ...
                    <td className="px-4 py-4 text-sm text-beige-700">{book.author}</td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4 text-sm text-beige-700">{book.stockQuantity}</td>
                    <td className="px-4 py-4">
                      <span
                ...
                <tr>
                  <td colSpan={10} className="px-4 py-12 text-center text-beige-500">
                    No books found
                  </td>
                </tr>
              )}
          ...
          <div className="px-6 py-4 border-t border-beige-200 flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="text-sm text-beige-600">
                Showing {startIndex + 1} to {Math.min(endIndex, filteredBooks.length)} of{" "}
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, promotionId: e.target.value || undefined })
                  }
                  className="w-full px-4 py-2 border border-beige-300 rounded-lg focus:ring-2 focus:ring-beige-500"
                >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        status: e.target.value as "In Stock" | "Out of Stock" | "Archived",
                      })
                    }
                    className="w-full px-4 py-2 border border-beige-300 rounded-lg focus:ring-2 focus:ring-beige-500"
                  >
