...
import { categories as categoriesData, type Category } from "../../data/categories";
  ...
  const getCategoryName = (categoryId: string) => {
    ...
    for (const mainCat of categories) {
      if (mainCat.subcategories) {
        const subCat = mainCat.subcategories.find((sc: any) => sc.id.toString() === categoryId);
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="max-h-64 overflow-y-auto border border-beige-300 rounded-lg p-3 space-y-2">
                    <label className="flex items-center gap-2 p-2 hover:bg-beige-50 rounded">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-sm font-semibold text-beige-900">All Products</span>
                    </label>
                    
                    {!formData.applyTo?.includes("all") && categories.map((mainCat) => (
                      <div key={mainCat.id} className="ml-4 space-y-1">
                        <label className="flex items-center gap-2 p-2 hover:bg-beige-50 rounded">
                          <input
                            type="checkbox"
                            checked={formData.applyTo?.includes(mainCat.id.toString())}
                            onChange={(e) => {
                              const currentApplyTo = formData.applyTo || [];
                              if (e.target.checked) {
                                setFormData({ 
                                  ...formData, 
                                  applyTo: [...currentApplyTo.filter(id => id !== "all"), mainCat.id.toString()] 
                                });
                              } else {
                                setFormData({ 
                                  ...formData, 
                                  applyTo: currentApplyTo.filter(id => id !== mainCat.id.toString()) 
                                });
                              }
                            }}
                            className="rounded border-beige-300 text-beige-600 focus:ring-beige-500"
                          />
                          <span className="text-sm font-medium text-beige-900">{mainCat.name}</span>
                        </label>
                        
                        {mainCat.subcategories && mainCat.subcategories.map((subCat) => (
                          <label key={subCat.id} className="flex items-center gap-2 p-2 ml-6 hover:bg-beige-50 rounded">
                            <input
                              type="checkbox"
                              checked={formData.applyTo?.includes(subCat.id.toString())}
                              onChange={(e) => {
                                const currentApplyTo = formData.applyTo || [];
                                if (e.target.checked) {
                                  setFormData({ 
                                    ...formData, 
                                    applyTo: [...currentApplyTo.filter(id => id !== "all"), subCat.id.toString()] 
                                  });
                                } else {
                                  setFormData({ 
                                    ...formData, 
                                    applyTo: currentApplyTo.filter(id => id !== subCat.id.toString()) 
                                  });
                                }
                              }}
                              className="rounded border-beige-300 text-beige-600 focus:ring-beige-500"
                            />
                            <span className="text-sm text-beige-700">↳ {subCat.name}</span>
                          </label>
                        ))}
                      </div>
                    ))}
