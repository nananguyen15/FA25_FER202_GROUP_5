...
export function Promotion() {
  ...
  const loadCategories = () => {
    const categoriesData = JSON.parse(localStorage.getItem("adminCategories") || "[]");
  ...
  const handleAddPromotion = () => {
    ...
    // Create notification
    const applyToText = newPromotion.applyTo === "all" 
      ? "all products" 
      : categories.find(c => c.id === newPromotion.applyTo)?.name || "selected category";
    
    createNotification(
      `New Promotion: ${newPromotion.name}`,
      `${newPromotion.name} - ${newPromotion.discountType === "percentage" ? newPromotion.discountValue + "%" : "$" + newPromotion.discountValue} off on ${applyToText}. Valid from ${new Date(newPromotion.startDate).toLocaleDateString()} to ${new Date(newPromotion.endDate).toLocaleDateString()}.`,
      "promotion"
    );
  ...
  const handleEditPromotion = () => {
    if (!selectedPromotion || !formData.name || !formData.startDate || !formData.endDate) {
      alert("Please fill in all required fields");
      return;
    }
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button
                          ...
                          className={promotion.active ? "text-green-600 hover:text-green-800" : "text-gray-600 hover:text-gray-800"}
                          title={promotion.active ? "Deactivate" : "Activate"}
                        >
                          {promotion.active ? <FaToggleOn size={20} /> : <FaToggleOff size={20} />}
                        </button>
                        <button
          ...
          <div className="bg-white rounded-lg max-w-md w-full">
            ...
            <div className="p-6">
              <p className="text-beige-700 mb-4">
                ...
                <strong>{selectedPromotion.name}</strong>? This action cannot be undone.
              </p>
            </div>
            <div className="p-6 border-t border-beige-200 flex gap-3 justify-end">
              <button
