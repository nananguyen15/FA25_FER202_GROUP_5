...
export function SeriesManagement() {
  ...
  useEffect(() => {
    applyFilters();
  }, [series, searchTerm, filterCategory, filterStatus, sortBy, sortOrder]);

  const loadData = () => {
    // Load series from localStorage
    const savedSeries = JSON.parse(localStorage.getItem("adminSeries") || "[]");
  ...
  const handleAddSeries = () => {
    ...
    localStorage.setItem("adminSeries", JSON.stringify(updatedSeries));
  ...
  const openDeleteModal = (item: Series) => {
    setSelectedSeries(item);
  ...
  const getCategoryName = (categoryId: string) => {
    ...
    const mainCat = categories.find((c: any) => c.id.toString() === categoryId);
    if (mainCat) return mainCat.name;
    
    // Check if it's a subcategory
    for (const mainCat of categories as any[]) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-beige-900">Manage Series</h2>
          <p className="text-beige-600 mt-1">Manage and monitor all series</p>
        </div>
        <button
          onClick={() => {
            setFormData({});
        ...
        >
          <FaPlus />
          <span className="font-medium">Add Series</span>
        </button>
      </div>
          ...
          <div className="px-6 py-4 border-t border-beige-200 flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="text-sm text-beige-600">
                ...
                {Math.min(endIndex, filteredSeries.length)} of{" "}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-beige-200 px-6 py-4 flex items-center justify-between">
              <h3 className="text-2xl font-bold text-beige-900">
                {showAddModal ? "Add New Series" : "Edit Series"}
              </h3>
              <button
                onClick={() => {
                  setShowAddModal(false);
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    {(categories as any[]).map((mainCat) => (
                      <optgroup key={mainCat.id} label={mainCat.name}>
                        <option value={mainCat.id}>{mainCat.name}</option>
                        {mainCat.subcategories && mainCat.subcategories.map((subCat: any) => (
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              ...
              <p className="mt-2 text-center text-beige-600">
                Are you sure you want to delete "{selectedSeries.title}"? This
