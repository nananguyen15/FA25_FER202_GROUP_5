...
interface CartContextType {
  ...
  updateQuantity: (id: string, type: 'book' | 'series', quantity: number) => void;
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const addToCart = (id: string, type: 'book' | 'series', quantity = 1) => {
    setCartItems(prevItems => {
      const existingItem = prevItems.find(item => item.id === id && item.type === type);
      if (existingItem) {
  ...
  const updateQuantity = (id: string, type: 'book' | 'series', quantity: number) => {
    setCartItems(prevItems =>
      prevItems.map(item =>
  ...
  const cartDetails = useMemo(() => {
    const itemsWithDetails = cartItems.map(cartItem => {
      const product = allProducts.find(p => p.id.toString() === cartItem.id && p.type === cartItem.type);
    ...
    return { itemsWithDetails, subtotal, promotionDiscount, shippingFee, total };
  }, [cartItems, allProducts]);

  return (
    <CartContext.Provider value={{ cartItems, addToCart, updateQuantity, removeFromCart, clearCart, cartDetails }}>
      {children}
