...
export function AuthProvider({ children }: { children: ReactNode }) {
      ...
      try {
        ...
        if (
          ...
          if (tokenData.username) {
            ...
            const userData = users.find((u: { username: string }) => u.username === tokenData.username);
            if (userData) {
              setUser({
  ...
  const login = (token: string, role: string = "user", username?: string) => {
    ...
    if (username) {
      const users = JSON.parse(localStorage.getItem("users") || "[]");
      const userData = users.find((u: { username: string }) => u.username === username);
      if (userData) {
        setUser({
          username: userData.username,
          email: userData.email,
  ...
  return (
    <AuthContext.Provider value={{ isAuthenticated, login, logout, userRole, user }}>
      {children}
